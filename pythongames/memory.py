...
ALL_SHAPES = (DOUGHNUT, SQUARE, DIAMOND, LINES, OVAL)
        ...
        if not revealed_boxes[box_x][box_y] and mouse_clicked:
          ...
          if first_selection == None:
            ...
            icon1_shape, icon1_color = get_shape_and_color \
              (main_board,first_selection[0], first_selection[1])
            icon2_shape, icon2_color = get_shape_and_color \
              (main_board,box_x, box_y)
...
def get_randomized_board():
  ...
  number_icons_used = int(BOARD_WIDTH * BOARD_HEIGHT / 2)
  icons = icons[0:number_icons_used] * 2
  random.shuffle(icons)
...
def get_box_at_pixel(x, y):
  for box_x in range(BOARD_WIDTH):
    for box_y in range(BOARD_HEIGHT):
      ...
      box_rectangle = pygame.Rect(left, top, BOX_SIZE, BOX_SIZE)
      if box_rectangle.collidepoint(x, y):
        return (box_x, box_y)
  return (None, None)
...
def draw_icon(shape, color, box_x, box_y):
  ...
  if shape == DOUGHNUT:
    ...
    pygame.draw.rect(DISPLAY_SURFACE, color, \
              (left + quarter, top + quarter, BOX_SIZE - half, BOX_SIZE - half))

  elif shape == DIAMOND:
    pygame.draw.polygon(DISPLAY_SURFACE, color, \
                      ((left + half, top),(left + BOX_SIZE - 1, top + half), \
                      (left + half, top + BOX_SIZE -1),(left, top + half)))

  elif shape == LINES:
    for i in range(0, BOX_SIZE, 4):
      pygame.draw.line(DISPLAY_SURFACE, color, \
        (left + i, top + BOX_SIZE - 1),(left + BOX_SIZE - 1, top + i))

  elif shape == OVAL:
    pygame.draw.ellipse(DISPLAY_SURFACE, color, (left, top + quarter,
      BOX_SIZE, half))
